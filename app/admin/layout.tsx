      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-3">
          <div className="space-y-1">
            {navItems.filter((item) => (item.href !== "/admin/users" && item.href !== "/admin/venue-settings" && item.href !== "/admin/sales" && item.href !== "/admin/manual-entries" && item.href !== "/admin/blocked-slots") || isAdmin).map((item) => (
              <Link
                key={item.href}
                href={item.href}
                onClick={() => setSidebarOpen(false)}
